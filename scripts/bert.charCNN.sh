...
for i in 0 1 2 3 4
	do
		python3 neural.py -m train --writePath $RESULTS_DIR/$MODEL.${i} -e ../data/guten.vectors.txt --pad --batchsize 16 --bert --charCNN
		python3 neural.py -m test --existingModel $RESULTS_DIR/$MODEL.${i}.hdf5 -e ../data/guten.vectors.txt --pad --batchsize 16 --bert --charCNN --predictionsFile $RESULTS_DIR/$MODEL.${i}.test.preds.txt
done
